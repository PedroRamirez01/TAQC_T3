...
class FilterProductPage:
    ...
    async def outOfStockFlow(self):
        ...
        total_value = []
        try:
            await self.outOfStock() #Apply out Stock filter
            await self.PopUp() #Close the Popup
            total_value = await self.addCart() # Attempts to add products to the cart and retrieves the total value
