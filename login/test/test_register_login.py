...
@pytest.mark.parametrize(
    ...
    [(user, user) for user in register_login_valid_new_users],
    indirect=["auto_delete_user"]
)
async def test_successful_registration_login(register_page: Page, login_page: Page, user: RegisterLoginUser, auto_delete_user: Callable):
    ...
    await login_page.login(user.login.email, user.login.password)
    assert "/my-account" in login_page.page.url, f"El inicio de sesión no fue exitoso: {user.login}"
    
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "user, auto_delete_user",
    [(user, user) for user in register_login_invalid_new_users],
    indirect=["auto_delete_user"]
)
async def test_successful_registration_login(register_page: Page, login_page: Page, user: RegisterLoginUser, auto_delete_user: Callable):
    await register_page.register(user.register)
